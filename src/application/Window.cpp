...
bool ApplicationWindow::Init(const std::string& title, uint32_t width, uint32_t height, bool fullscreen, bool windowed, bool vsync, uint32_t msaa,
                             uint32_t r, uint32_t g, uint32_t b, uint32_t alpha, uint32_t depth, uint32_t stencil) {
  if (!glfwInit()) {
