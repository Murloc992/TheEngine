...
class GUIGraph : public GUIElement {
 public:
  GUIGraph(GUIEnvironment* env, Rect2D<int> dimensions, bool drawBorders = true, bool drawCenter = true, bool drawAxisValues = true,
           bool drawBackground = true, bool drawChart = false);
  ~GUIGraph();

  void AddValue(const std::string& valueName, float value, int spacing = 1, const glm::vec4& color = glm::vec4(255),
                bool trackMinMax = false);  // last add time..?
