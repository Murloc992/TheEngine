...
GUIGraph::GUIGraph(GUIEnvironment* env, Rect2D<int> dimensions, bool drawBorders, bool drawCenter, bool drawAxisValues, bool drawBackground,
                   bool drawChart)
...
void GUIGraph::AddValue(const std::string& valueName, float value, int spacing, const glm::vec4& color, bool trackMinMax) {
  ...
  if (properties.trackMinMax) {
    ...
    wchar_t buf[256];
#ifdef __linux__
    swprintf(buf, 255, L"['s]['c %d %d %d]%s Min:%4.4f Max:%4.4f Current:%4.4f[c'][s']", (int)properties.color.r, (int)properties.color.g,
             (int)properties.color.b, helpers::to_wstr(valueName.c_str()).c_str(), properties.minValue, properties.maxValue, value);
#else
    swprintf(buf, L"['s]['c %d %d %d]%s Min:%4.4f Max:%4.4f Current:%4.4f[c'][s']", (int)properties.color.r, (int)properties.color.g,
             (int)properties.color.b, helpers::to_wstr(valueName.c_str()).c_str(), properties.minValue, properties.maxValue, value);
#endif
