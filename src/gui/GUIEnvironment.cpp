...
GUIEnvironment::GUIEnvironment()
  : GUIElement(nullptr, Rect2D<int>(0, 0, GetContext().GetWindow()->GetWindowSize().x, GetContext().GetWindow()->GetWindowSize().y)) {
...
GUIEditBox* GUIEnvironment::AddGUIEditBox(Rect2D<int> dimensions, std::wstring text, glm::vec4 text_color, bool drawbackground, bool drawshadow,
                                          bool clearonsubmit) {
...
GUISlider* GUIEnvironment::AddGUISlider(Rect2D<int> dimensions, float min, float max, float pos, bool drawValue, bool snap, bool wide, bool vertical,
                                        bool dark) {
  ...
  if (target != nullptr) {
    if (target->IsEnabled() && target->IsVisible() && target->AcceptsEvents()) {
      if (target != hover) {
        if (hover != nullptr) {
          last_hover = hover;
          last_hover->OnEvent(GUIEvent(gui_event_type::element_exitted, this, last_hover));
          last_hover->SetHovered(false);
        }
        hover = target;
        hover->OnEvent(GUIEvent(gui_event_type::element_hovered, this, hover));
  ...
  switch (action) {
    case GLFW_PRESS:
      ...
      if (focus != nullptr && hover == focus) {
        gui_event_type eventType = button == GLFW_MOUSE_BUTTON_LEFT  ? left_mouse_pressed
                                 : button == GLFW_MOUSE_BUTTON_RIGHT ? right_mouse_pressed
    ...
    case GLFW_RELEASE:
      ...
      if (focus != nullptr && hover == focus) {
        gui_event_type eventType = button == GLFW_MOUSE_BUTTON_LEFT  ? left_mouse_released
                                 : button == GLFW_MOUSE_BUTTON_RIGHT ? right_mouse_released
