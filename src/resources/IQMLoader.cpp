...
void IQMLoader::loadiqmanims(std::shared_ptr<Mesh> m, const char* data, iqmheader& header)
{
	...
	for (int i = 0; i < (int)header.num_anims; i++)
	{
		...
		{
			if (bones[i].parent < 0) continue; // this is a root bone
		...
		m_logger->log(LOG_DEBUG, "Loaded anim: %s. start: %d frames: %d framerate: %d loop: %s", ai.name.c_str(), ai.start, ai.num, ai.framerate, ai.loop?"true":"false");
	}
}