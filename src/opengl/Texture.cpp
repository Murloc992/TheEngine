...
void Texture::InitArray(ImagePtr img, uint32_t tile_size_x, uint32_t tile_size_y) {
  ...
  for (int32_t x = 0; x < tile_count_x; x++) {
    for (int32_t y = 0; y < tile_count_y; y++) {
      ...
      glTexSubImage3D(Type, 0, 0, 0, (tile_count_x - 1 - x) * tile_count_y + y, tile_size_x, tile_size_y, 1, imageFormat, dataType,
...
void Texture::SetFilters(TextureFilterMin fmin, TextureFilterMag fmag) {
  ...
  glTexParameteri(
    ...
    fmin == TextureFilterMin::LINEAR_MIPMAP
      ? GL_LINEAR_MIPMAP_LINEAR
      : (fmin == TextureFilterMin::NEAREST_MIPMAP ? GL_NEAREST_MIPMAP_NEAREST : (fmin == TextureFilterMin::LINEAR ? GL_LINEAR : GL_NEAREST)));
...
void Texture::SetClampMode(TextureClamp x, TextureClamp y, TextureClamp z) {
  ...
  if (Type == GL_TEXTURE_3D) {
    glTexParameteri(Type, GL_TEXTURE_WRAP_R,
                    y == TextureClamp::EDGE ? GL_CLAMP_TO_EDGE : (y == TextureClamp::BORDER ? GL_CLAMP_TO_BORDER : GL_REPEAT));
  }
...
void Texture::SetSubImage3D(const uint8_t* data, uint32_t xoffset, uint32_t yoffset, uint32_t zoffset, uint32_t width, uint32_t height,
                            uint32_t depth, uint32_t overrideFormat) {
  glActiveTexture(GL_TEXTURE0 + active_slot);
  if (current != Id)
    glBindTexture(Type, Id);
