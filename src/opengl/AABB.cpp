...
bool AABB::ContainsPoint(const glm::vec3& point) const {
  ...
  return (glm::abs(distance.x) <= m_halfSize.x) && (glm::abs(distance.y) <= m_halfSize.y) && (glm::abs(distance.z) <= m_halfSize.z);
}
...
bool AABB::IntersectsWith(const glm::vec3& center, const glm::vec3& halfsize) const {
  return (glm::abs(m_center.x - center.x) <= m_halfSize.x + halfsize.x) && (glm::abs(m_center.y - center.y) <= m_halfSize.y + halfsize.y) &&
