...
class SlicedGUIQuad {
 private:
  ...
  std::vector<glm::vec3> CreateVerts() {
    ...
    verts[1] = glm::vec3(-m_size.x + m_margin * ratio.x, -m_size.y, 0);
    verts[2] = glm::vec3(-m_size.x + m_margin * ratio.x, -m_size.y + m_margin * ratio.y, 0);
    verts[3] = glm::vec3(-m_size.x, -m_size.y + m_margin * ratio.y, 0);
    /// ///////////////////////////////////////////////////////

    /// LR
    verts[4] = glm::vec3(m_size.x, -m_size.y, 0);

    verts[5] = glm::vec3(m_size.x - m_margin * ratio.x, -m_size.y, 0);
    verts[6] = glm::vec3(m_size.x - m_margin * ratio.x, -m_size.y + m_margin * ratio.y, 0);
    verts[7] = glm::vec3(m_size.x, -m_size.y + m_margin * ratio.y, 0);
    /// ///////////////////////////////////////////////////////

    /// UR
    verts[8] = glm::vec3(m_size.x, m_size.y, 0);

    verts[9] = glm::vec3(m_size.x - m_margin * ratio.x, m_size.y, 0);
    verts[10] = glm::vec3(m_size.x - m_margin * ratio.x, m_size.y - m_margin * ratio.y, 0);
    verts[11] = glm::vec3(m_size.x, m_size.y - m_margin * ratio.y, 0);
    /// ///////////////////////////////////////////////////////
    ...
    verts[14] = glm::vec3(-m_size.x + m_margin * ratio.x, m_size.y - m_margin * ratio.y, 0);
    verts[15] = glm::vec3(-m_size.x, m_size.y - m_margin * ratio.y, 0);
    /// ///////////////////////////////////////////////////////
    ///
    return verts;
  }
  ...
  void CreateInds() {
    const static uint32_t slice_indices[54] {
      // LL corner
      1,
      3,
      0,
    ...
    indices->data.insert(indices->data.begin(), std::begin(slice_indices), std::end(slice_indices));
  }
 ...
 public:
  ...
  std::vector<glm::vec2> CreateTCoords(const std::vector<glm::vec2>& uvs) {
    ...
    coords[14] = glm::vec2(d.x + tm.x, d.y - tm.y);
    coords[15] = glm::vec2(d.x, d.y - tm.y);
