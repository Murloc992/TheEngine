...
Var& VarGroup::GetVar(const char* name) {
  auto it = std::find_if(m_vars.begin(), m_vars.end(), [&name](const Var& var) {
...
VarGroup& VarGroup::AddGroup(const char* name) {
  std::vector<VarGroup>::iterator it = std::find_if(m_groups.begin(), m_groups.end(), [&name](const VarGroup& varGroup) {
...
VarGroup& VarGroup::AddGroup(const VarGroup& vGroup) {
  ...
  auto it = std::find_if(m_groups.begin(), m_groups.end(), [&vGroup](const VarGroup& varGroup) {
...
bool VarGroup::AddVar(const Var& variable) {
  std::vector<Var>::iterator it = std::find_if(m_vars.begin(), m_vars.end(), [&variable](const Var& var) {
